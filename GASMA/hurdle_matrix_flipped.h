...
private:
    ...
    highways* long_highway_list;


protected:
    // maximum differ ence allowed
    int k;
    ...
    /**
     * Update long_highway_list for each lane and show the closest highway to the
     * current position. Calculate the cost and record the longest highway.
     *
     * Must be called after _update_highway_list().
     *
     */
    bool _update_long_highway_list() {
        int largest_total_heuristic = INT32_MIN, largest_leap_heuristic = INT32_MIN;
        int best_highway_lane = 0;
        int first_zero;
        bool reaching_destination = false; // check if we are reaching destination
        for (int lane = -k; lane <= k; lane++) {
            int start_col = current_column + linear_leap_forward_column(current_lane, lane);
            if ((*long_highway_list)[lane].starting_point < start_col) {
                // get closest highway in the lane
                int_128bit l = (long_highway_lanes[lane + k]).shift_left(start_col);

                // update highway in lane
                first_zero = l.first_zero();
                int next_hurdle = (l.shift_left(first_zero)).first_one();
                (*long_highway_list)[lane].starting_point = start_col + first_zero;
                (*long_highway_list)[lane].length = next_hurdle;

                // Fix length if reaches destination
                if (start_col + first_zero + next_hurdle > (*highway_list)[lane].destination) {
                    (*long_highway_list)[lane].length = std::max(0, (*highway_list)[lane].destination -\
                                                   (start_col + first_zero));
                    reaching_destination = true;
                }
            }
            ...
            // FIXME: use function pointer for more complicated penalties.
            int leap_cost = linear_leap_lane_penalty(current_lane, lane);
            int hurdle_cost = (*long_highway_list)[lane].starting_point - start_col;
            (*long_highway_list)[lane].leap_cost = leap_cost;
            (*long_highway_list)[lane].hurdle_cost = hurdle_cost;
        ...
        for (int lane = -k; lane <= k; lane++) {
            ...
            if (reaching_destination) {
                ...
                heuristic_2 -= final_leap_cost + ((*highway_list)[lane].destination -
            ...
            if (heuristic_1 > heuristic_2) {
                ...
                (*long_highway_list)[lane].hurdle_cost = (*highway_list)[lane].hurdle_cost;
        ...
        long_highway_list->best_highway_lane = best_highway_lane;
#ifdef DEBUG
        long_highway_list->print();
        printf("Best highway lane: %d\n", best_highway_lane);
#endif
        if ((*long_highway_list)[best_highway_lane].length <= 0) {
            return false;
        }
        return true;
    }
    ...
    virtual int _choose_best_highway() {
        ...
        int best_lane = long_highway_list->best_highway_lane;
        int starting_point = (*long_highway_list)[best_lane].starting_point;
        int best_lane_cost = (*long_highway_list)[best_lane].hurdle_cost + (*long_highway_list)[best_lane].leap_cost;
    ...
    void _construct_hurdles() {
        ...
        for (int lane = -k; lane <= k; lane++) {
            ...
            (*this)[lane] = mask;
            long_highway_lanes[lane + k] = mask.flip_short_matches(1);
        }
    }
...
public:
    ...
    hurdle_matrix_flipped(const char* read, const char* ref, int error) {
        ...
        long_highway_list = new highways(MAX_K, m, n);
        lanes = new int_128bit[2 * MAX_K + 1];
        long_highway_lanes = new int_128bit[2 * MAX_K + 1];
    ...
    ~hurdle_matrix_flipped() {
        delete highway_list;
        delete long_highway_list;
        delete[] lanes;
        delete[] long_highway_lanes;
    }
};
