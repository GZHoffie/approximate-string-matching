...
#ifndef GASMA_HURDLE_MATRIX_PROJECTED_H
#define GASMA_HURDLE_MATRIX_PROJECTED_H
...
protected:
    ...
    bool _update_highway_list() {
        ...
        int highway_cost = 0, highway_project = 0;
        for (int lane = -k; lane <= k; lane++) {
            // Calculate total cost
            highway_cost = (*highway_list)[lane].leap_cost + (*highway_list)[lane].hurdle_cost;

            // Calculate the projected highway length
            highway_project = (m + n) * ((*highway_list)[lane].hurdle_cost + (*highway_list)[lane].length);
            if (lane < current_lane) {
                highway_project += m * (*highway_list)[lane].leap_cost;
            } else {
                highway_project += n * (*highway_list)[lane].leap_cost;
            }

            // get the best-looking highway
            if (reaching_destination) {
                int final_leap_cost = linear_leap_lane_penalty(lane, destination_lane);
                int final_hurdle_cost = (*highway_list)[lane].destination -
                                        (*highway_list)[lane].starting_point - (*highway_list)[lane].length;
                highway_cost += final_leap_cost + final_hurdle_cost;
                if (destination_lane < lane) {
                    highway_project += m * final_leap_cost + (m + n) * final_hurdle_cost;
                } else {
                    highway_project += n * final_leap_cost + (m + n) * final_hurdle_cost;
                }
            }
            heuristic = highway_cost / highway_project;
            // printf("lane %d: heuristic %d\n", lane, heuristic);
            if (heuristic < smallest_cost_density) {
                smallest_cost_density = heuristic;
...
public:
    ...
    hurdle_matrix_projected(const char* read, const char* ref, int error) {
